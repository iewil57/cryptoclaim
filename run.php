...
system("termux-open-url https://youtu.be/tOMA6Kl7Isk");
bn();
cookie:
...
$ua1=['Host:'.host,'User-agent:'.$user_agent,'Cookie:'.$cookie,'Referer:https://'.host.'/surf'];
$ua2=['Host:'.host,'User-agent:'.$user_agent,'Cookie:'.$cookie,'Origin:https://'.host,'referer:https://'.host.'/surf/'.$id];
$ua3=['Host:'.host,'user-agent:'.$user_agent,'cookie:'.$cookie,'accept:text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','referer:https://'.host.'/surf/'.$id,'accept-encoding:gzip, deflate','accept-language:id,id-ID;q=0.9,en-US;q=0.8,en;q=0.7'];
$ua4=['Host:'.host,'accept:*/*','x-requested-with:XMLHttpRequest','user-agent:'.$user_agent,'content-type:application/x-www-form-urlencoded; charset=UTF-8','origin:https://'.host,'sec-fetch-site:same-origin','referer:https://'.host.'/surf/'.$id,'accept-encoding:gzip, deflate','accept-language:id,id-ID;q=0.9,en-US;q=0.8,en;q=0.7','cookie:'.$cookie];
...
while(true){
	...
	if($tmr){tmr($tmr);goto ngok;}else{
		...
		$r4=Run('https://'.host.'/faucet',$ua,$data);
...
while(true){
	...
	if($tmr){tmr($tmr);goto manual;}else{
		...
		$coba=Run('https://'.host.'/captcha/'.$id,$ua);
